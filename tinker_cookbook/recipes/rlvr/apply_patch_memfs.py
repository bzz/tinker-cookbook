...
""""
Adapted from https://github.com/openai/openai-cookbook/blob/b3ee6e549d35e4c3de372375ed5374ae75c79992/examples/gpt-5/apply_patch.py
main() has an example of how to use FS mock for multi-file patches with https://github.com/PyFilesystem/pyfilesystem2
"""
...
def find_context_core(lines: list[str], context: list[str], start: int) -> tuple[int, int]:
    if not context:
        # When there is no context, default to appending just before the
        # trailing newline (if present). This matches patches that only add
        # lines at EOF while preserving the file's final newline.
        idx = len(lines)
        if idx > 0 and lines[-1] == "":
            idx -= 1
        return idx, 0
...
def process_patch(text: str, open_fn: Callable, write_fn: Callable, remove_fn: Callable) -> Commit:
...
def apply_patch_to_text(old: str, patch_text: str) -> str:
    """Apply a single-file Update File patch to the provided in-memory text.
    Returns the updated file contents as a string.
    """
    ...
    # Parse and compute new contents purely in-memory
    patch, _fuzz = text_to_patch(patch_text, orig)
    commit = patch_to_commit(patch, orig)
    change = commit.changes.get(target)
    if not change or change.type != ActionType.UPDATE or change.new_content is None:
        raise DiffError("Failed to compute updated content for the target file")
...
def main(args: argparse.Namespace):
    ...
    def _create_memfs():
        """Create a minimal memfs example that can work with apply_patch.py"""

        # Create in-memory filesystem
        memfs = MemoryFS()

        # Write some initial files
        with memfs.open("hello.py", "w") as f:
            f.write('print("Hello")\n')

        with memfs.open("README.md", "w") as f:
            f.write("# My Project\n\nOriginal content.\n")

        # Create adapter functions compatible with apply_patch.py
        def memfs_open_file(path: str) -> str:
            """Open and read file from memfs"""
            with memfs.open(path, "r") as f:
                return f.read()

        def memfs_write_file(path: str, content: str) -> None:
            """Write file to memfs"""
            if path.startswith("/"):
                print("We do not support absolute paths.")
                return
            ...
            if "/" in path:
                parent = "/".join(path.split("/")[:-1])
                memfs.makedirs(parent, recreate=True)

            with memfs.open(path, "w") as f:
                f.write(content)

        def memfs_remove_file(path: str) -> None:
            """Remove file from memfs"""
            memfs.remove(path)

        return memfs, memfs_open_file, memfs_write_file, memfs_remove_file

    memfs, open_file, write_file, remove_file = _create_memfs()
    patch_text = sys.stdin.read()
    if not patch_text:
        print("Please pass patch text through stdin")
        return
    try:
        # write old_file to old_file_path in FS before applying patch
        old_file_content = Path(args.old_file).read_text(encoding="utf-8")
        write_file(args.old_file_path, old_file_content)

        commit = process_patch(patch_text, open_file, write_file, remove_file)
    except DiffError as e:
        print(str(e))
        return
    print(commit.changes.get(args.old_file_path))


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--old_file_path", type=str, required=True)
