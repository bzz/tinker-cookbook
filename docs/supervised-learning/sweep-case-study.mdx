...
This example runs the existing `sl_loop.py` recipe (no copy/pasted training code) and caps each trial to a fixed number of steps (`max_steps`) so iterations are fast:
