...
## Sweeping DPO hyperparameters
For DPO, the most important hyperparameters to sweep are usually:
- **`learning_rate`** (often \(10^{-6}\)–\(10^{-5}\) for DPO)
- **`dpo_beta`** (often \(0.05\)–\(0.5\))
- **`batch_size`** (tradeoff between stability/throughput)

### Bayesian optimization + tmux launcher (Optuna + xmux)
For multi-parameter sweeps, Bayesian optimization can find good configurations with fewer trials than grid search. This repo includes a runnable example that:
- Uses **Optuna** to propose DPO hyperparameters
- Launches each trial as a separate job in a **tmux** session via `xmux`
- Reads a scalar metric from `metrics.jsonl` and reports it back to Optuna

See:
<CookbookLink path="tinker_cookbook/recipes/preference/dpo/xmux_optuna_sweep.py">xmux_optuna_sweep.py</CookbookLink>
...
Run (example optimizing `test/nll`):
